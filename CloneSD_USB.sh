...
FileName=SD-Backup_$BoxToClone.img.gz
File=/usb/SD/$FileName

# copie de la Base à froid
...
if [ $? -eq 0 ]; then
        if [ $FileStat -ge $SdImgSize ]; then
                Objet="($DATE) SD Backup $BoxToClone sur USB : ERREUR"
                Message="Taille du fichier $FileName incorrecte"
        else
                #taille OK
                Objet="($DATE) SD-Backup $BoxToClone sur USB : OK"
                Message="Clonage OK"
        fi
else
        if [ -e $File ]; then
        ...
        Objet="($DATE) SD-Backup $BoxToClone sur USB : *ECHEC*"
